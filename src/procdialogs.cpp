...
procdialog_create_kill_dialog (ProcData *procdata, int signal)
{
    ...
    kill_alert_dialog = ctk_message_dialog_new (GTK_WINDOW (procdata->app),
    ...
    ctk_message_dialog_format_secondary_text (GTK_MESSAGE_DIALOG (kill_alert_dialog),
                                              "%s",
                                              secondary);

    ctk_dialog_add_buttons (GTK_DIALOG (kill_alert_dialog),
                            "ctk-cancel", GTK_RESPONSE_CANCEL,
                            button_text, GTK_RESPONSE_OK,
                            NULL);

    ctk_dialog_set_default_response (GTK_DIALOG (kill_alert_dialog),
                                     GTK_RESPONSE_CANCEL);
...
procdialog_create_renice_dialog (ProcData *procdata)
{
    ...
    ctk_window_set_resizable (GTK_WINDOW (renice_dialog), FALSE);
    ctk_container_set_border_width (GTK_CONTAINER (renice_dialog), 5);

    button = ctk_button_new_with_mnemonic (_("Change _Priority"));
    ctk_widget_set_can_default (button, TRUE);

    icon = ctk_image_new_from_stock ("ctk-ok", GTK_ICON_SIZE_BUTTON);
    ctk_button_set_image (GTK_BUTTON (button), icon);

    ctk_dialog_add_action_widget (GTK_DIALOG (renice_dialog), button, 100);
    ctk_dialog_set_default_response (GTK_DIALOG (renice_dialog), 100);
    new_nice_value = -100;
    ...
    ctk_grid_attach (GTK_GRID (grid), label, 0, 0, 1, 2);

    renice_adj = ctk_adjustment_new (info->nice, RENICE_VAL_MIN, RENICE_VAL_MAX, 1, 1, 0);
    new_nice_value = 0;
    hscale = ctk_scale_new (GTK_ORIENTATION_HORIZONTAL, renice_adj);
...
field_toggled (const gchar *child_schema, GtkCellRendererToggle *cell, gchar *path_str, gpointer data)
{
    ...
    ctk_list_store_set (GTK_LIST_STORE (model), &iter, 0, !toggled, -1);
...
create_field_page(GtkWidget *tree, const gchar *child_schema, const gchar *text)
{
    ...
    treeview = ctk_tree_view_new_with_model (GTK_TREE_MODEL (model));
...
procdialog_create_preferences_dialog (ProcData *procdata)
{
    ...
    ctk_window_set_default_size (GTK_WINDOW (dialog), 400, 500);
    ...
    ctk_container_set_border_width (GTK_CONTAINER (notebook), 5);
    ctk_box_pack_start (GTK_BOX (main_vbox), notebook, TRUE, TRUE, 0);

    proc_box = ctk_box_new (GTK_ORIENTATION_VERTICAL, 18);
    ctk_container_set_border_width (GTK_CONTAINER (proc_box), 12);
    tab_label = ctk_label_new (_("Processes"));
    ctk_widget_show (tab_label);
    ctk_notebook_append_page (GTK_NOTEBOOK (notebook), proc_box, tab_label);

    vbox = ctk_box_new (GTK_ORIENTATION_VERTICAL, 6);
    ctk_box_pack_start (GTK_BOX (proc_box), vbox, FALSE, FALSE, 0);
    ...
    ctk_box_pack_start (GTK_BOX (vbox2), hbox2, FALSE, FALSE, 0);

    label = ctk_label_new_with_mnemonic (_("_Update interval in seconds:"));
    ctk_label_set_xalign (GTK_LABEL (label), 0.0);
    ctk_box_pack_start (GTK_BOX (hbox2), label, FALSE, FALSE, 0);

    hbox3 = ctk_box_new (GTK_ORIENTATION_HORIZONTAL, 6);
    ctk_box_pack_start (GTK_BOX (hbox2), hbox3, TRUE, TRUE, 0);
    ...
    ctk_box_pack_start (GTK_BOX (hbox3), spin_button, FALSE, FALSE, 0);
    ...
    hbox2 = ctk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
    ctk_box_pack_start(GTK_BOX(vbox2), hbox2, FALSE, FALSE, 0);

    smooth_button = ctk_check_button_new_with_mnemonic(_("Enable _smooth refresh"));
    ctk_toggle_button_set_active(GTK_TOGGLE_BUTTON(smooth_button),
    ...
    ctk_box_pack_start(GTK_BOX(hbox2), smooth_button, TRUE, TRUE, 0);
    ...
    ctk_box_pack_start (GTK_BOX (hbox2), check_button, FALSE, FALSE, 0);
    ...
    ctk_toggle_button_set_active(GTK_TOGGLE_BUTTON(solaris_button),
                                 g_settings_get_boolean(procdata->settings,
    ...
    ctk_box_pack_start(GTK_BOX(hbox2), solaris_button, TRUE, TRUE, 0);
    ...
    vbox = ctk_box_new (GTK_ORIENTATION_VERTICAL, 6);
    ctk_box_pack_start (GTK_BOX (proc_box), vbox, TRUE, TRUE, 0);
    ...
    sys_box = ctk_box_new (GTK_ORIENTATION_VERTICAL, 12);
    ctk_container_set_border_width (GTK_CONTAINER (sys_box), 12);
    tab_label = ctk_label_new (_("Resources"));
    ctk_widget_show (tab_label);
    ctk_notebook_append_page (GTK_NOTEBOOK (notebook), sys_box, tab_label);

    vbox = ctk_box_new (GTK_ORIENTATION_VERTICAL, 6);
    ctk_box_pack_start (GTK_BOX (sys_box), vbox, FALSE, FALSE, 0);
    ...
    ctk_toggle_button_set_active(GTK_TOGGLE_BUTTON(bits_button),
                                 g_settings_get_boolean(procdata->settings,
    ...
    ctk_box_pack_start(GTK_BOX(vbox2), bits_button, TRUE, TRUE, 0);



    hbox2 = ctk_box_new (GTK_ORIENTATION_HORIZONTAL, 12);
    ctk_box_pack_start (GTK_BOX (vbox2), hbox2, TRUE, TRUE, 0);
    ...
    ctk_widget_show (tab_label);
    ctk_notebook_append_page (GTK_NOTEBOOK (notebook), vbox, tab_label);
    ...
    ctk_box_pack_start (GTK_BOX (hbox), vbox3, TRUE, TRUE, 0);
    ...
    case PROCMAN_TAB_DISKS:
        ctk_notebook_set_current_page(GTK_NOTEBOOK(notebook), 2);
        break;
